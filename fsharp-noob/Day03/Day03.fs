...
//let test_input = """............409..........784...578...802......64..............................486.248..............177....................369...............
...
let extract_surrounding_text (offset: int, part: Match, input: string) : string =
    let index = part.Groups.[0].Index + offset
    let length = part.Groups.[0].Length
    let start = index - 1
    let end_ = index + length
    let text = input[start..end_]
    text.Replace("\n", "").Trim()


let validate_part (line_length: int, part: Match, input: string) : EnginePart =
    // Check if the match has symbols close by getting surrounding text
    let above_text = extract_surrounding_text(-line_length, part, input)
    let middle_text = extract_surrounding_text(0, part, input)
    let below_text = extract_surrounding_text(line_length, part, input)
    
    let has_symbol = Regex.IsMatch(above_text + middle_text + below_text, "[^0-9.]")

    let result = {
        PartNumber = int part.Value
        SurroundingText = above_text + "\n" + middle_text + "\n" + below_text
        Range = [part.Index .. part.Index + part.Length - 1]
        HasSymbol = has_symbol }
    result
    
    
let parse_parts (input: string) : EnginePart seq =
    let line_length = input.IndexOf('\n') + 1
    Regex.Matches(input, "(\d+)")
        |> Seq.cast<Match>
        |> Seq.map (fun item -> validate_part(line_length, item, input))


let find_gears (input: string) : Gear seq =
    Regex.Matches(input, "\*")
        |> Seq.cast<Match>
        |> Seq.map (fun item -> { Position = item.Index })

        
let solve_part_1 (input: string) : int =
    input
        |> parse_parts
        |> Seq.filter (fun item -> item.HasSymbol)
        |> Seq.sumBy (fun item -> item.PartNumber)
        

let sum_gears (input: string) : int =
    //let p = input
    //    |> parse_parts
    //    |> Seq.filter (fun item -> item.HasSymbol)
